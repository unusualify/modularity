<template>
  <div class="media" >
    <!-- <div class="media__field">
      <div class="media__info" v-if="hasMedia">
        ...
        <ul class="media__metadatas" v-if="!disabled">
          ...
          <li class="f--small" v-if="media.width + media.height">{{ $trans('fields.medias.original-dimensions') }}: {{
            media.width }}&nbsp;&times;&nbsp;{{
    media.height }}
          </li>
          <li class="f--small media__crop-link" v-if="cropInfos && activeCrop" @click="openCropMedia">
            <p class="f--small f--note hide--xsmall" v-for="(cropInfo, index) in cropInfos" :key="index">
              <span v-html="cropInfo"></span>
            </p>
          </li>
          <li class="f--small">
            <a href="#" @click.prevent="metadatasInfos" v-if="withAddInfo" class="f--link-underlined--o">{{ metadatas.text
            }}</a>
          </li>
        </ul> -->
        ...
        <div class="media__actions-dropDown">
          <a17-dropdown ref="dropDown" position="right">
            ...
            <div slot="dropdown__content">
              <a :href="media.original" download><span v-svg symbol="download"></span>{{ $trans('fields.medias.download')
              }}</a>
              <button type="button" @click="openCropMedia" v-if="activeCrop"><span v-svg symbol="crop"></span>{{
                $trans('fields.medias.crop') }}
              </button>
              <button type="button" @click="deleteMediaClick"><span v-svg symbol="trash"></span>{{
                $trans('fields.medias.delete') }}</button>
            </div>
          </a17-dropdown>
        </div>
      </div> -->
      ...
      <!--Add media button-->
      <!-- <a17-button variant="ghost" @click="openMediaLibrary" :disabled="disabled" v-if="!hasMedia">{{ buttonLabel
      }}</a17-button>
      ...
      <!-- <div class="media__metadatas--options" :class="{ 's--active': metadatas.active }" v-if="hasMedia && withAddInfo">
        <a17-mediametadata :name='metadataName' :label="$trans('fields.medias.alt-text', 'Alt Text')" id="altText"
          :media="media" :maxlength="altTextMaxLength" @change="updateMetadata" />

        <a17-mediametadata v-if="withCaption" :wysiwyg="useWysiwyg" :wysiwyg-options="wysiwygOptions" type='text'
          :name='metadataName' :label="$trans('fields.medias.caption', 'Caption')" id="caption" :media="media"
          :maxlength="captionMaxLength" @change="updateMetadata" />

        <a17-mediametadata v-if="withVideoUrl" :name='metadataName'
          :label="$trans('fields.medias.video-url', 'Video URL (optional)')" id="video" :media="media"
          @change="updateMetadata" />

        <template v-for="field in extraMetadatas">
          <a17-mediametadata v-if="extraMetadatas.length > 0" :key="field.name" :type="field.type" :name='metadataName'
            :wysiwyg='field.wysiwyg || false' :wysiwyg-options='field.wysiwygOptions || wysiwygOptions'
            :label="field.label" :id="field.name" :media="media" :maxlength="field.maxlength || 0"
            @change="updateMetadata" />
        </template>
      </div>
    </div> -->
    ...
    <!-- Crop modal -->
    <!-- <a17-modal class="modal--cropper" :ref="cropModalName" :forceClose="true" :title="$trans('fields.medias.crop-edit')"
      mode="medium" v-if="hasMedia && activeCrop">
      <a17-cropper :media="media" v-on:crop-end="cropMedia" :aspectRatio="16 / 9" :context="cropContext"
        :key="cropperKey">
        <a17-button class="cropper__button" variant="action" @click="$refs[cropModalName].close()">{{
          $trans('fields.medias.crop-save') }}</a17-button>
      </a17-cropper>
    </a17-modal> -->
    <input name="test123" type="hidden" >
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    btnLabel: {
      type: String,
      // default() {
      //   return t('fields.medias.btn-label', 'Attach image')
      // }
    },
    hover: {
      type: Boolean,
  ...
  data: function () {
    return {
      ...
      buttonLabel : this.btnLabel ? this.btnLabel : this.$t('fields.medias.btn-label', 'Attach image')
    }
  },
