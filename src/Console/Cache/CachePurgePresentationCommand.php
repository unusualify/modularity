...
final class CachePurgePresentationCommand extends BaseCommand
{
    /**
     * @var string
     */
    protected $signature = 'modularous:cache:purge-presentation
                            {--module= : Module name (StudlyCase), e.g. Blog}
                            {--route= : Route name (StudlyCase); requires --module}
                            {--locale= : Purge URL path variants for a single locale only}
                            {--sync : Purge synchronously instead of dispatching queue jobs}
                            {--queue : Deprecated; queue dispatch is now the default}
                            {--chunk=100 : Batch size when iterating published models}
                            {--id= : Purge a single record id}
                            {--dry-run : List what would be purged without deleting files or dispatching jobs}';

    /**
     * @var string
     */
    protected $description = 'Purge public presentationItem filesystem caches (model-id stale + URL store)';
    ...
    public function handle(): int
    {
        ...
        if ($dryRun) {
            $this->info('Dry run — listing targets that would be purged.');
        } else {
            ModularousCacheLogger::info('cache.command.purge_presentation.start', [
                'module' => $moduleFilter,
                'route' => $routeFilter,
                'locale' => $locale,
                'queue' => $useQueue,
        ...
        foreach ($targets as $target) {
            ...
            $this->info("{$moduleName} -> {$routeName}: purging presentationItem caches" . ($useQueue ? ' (queue)' : ''));

            if ($useQueue && $recordId === null) {
                PurgeModulePresentationCachesJob::dispatch($moduleName, $routeName, $chunkSize);
                $queuedJobs++;
                $this->line('  <fg=green>✓</> Queued route purge job');
            ...
            $query->chunk($chunkSize, function ($records) use (
                ...
                foreach ($records as $record) {
                    ...
                    if ($useQueue) {
                        PurgePresentationItemJob::dispatch($record, $moduleName, $routeName, $locale);
                        $queuedJobs++;
                    } else {
                        $deleted = $cacheService->purgePresentationItemForModel(
                            $record,
                            $moduleName,
                            $routeName,
                            $locale,
        ...
        if ($dryRun) {
            ...
            $this->info("Purged {$purged} record(s) ({$filesDeleted} file(s)); skipped {$skipped}.");
        }

        if (! $dryRun) {
            ModularousCacheLogger::info('cache.command.purge_presentation.complete', [
                'module' => $moduleFilter,
                'route' => $routeFilter,
                'queue' => $useQueue,
    ...
    protected function reportDryRunTarget(
        ...
        if ($useQueue && $recordId === null) {
            $this->line("Would queue PurgeModulePresentationCachesJob: {$moduleName}::{$routeName} ({$total} records) [{$modeLabel}, locale={$localeLabel}]");
            ...
            $query->chunk($chunkSize, function ($records) use (
                ...
                foreach ($records as $record) {
                    ...
                    if ($useQueue) {
                        ...
                        $this->line("Would queue PurgePresentationItemJob: {$moduleName}::{$routeName} {$modelLabel} (id={$record->getKey()}){$localeSuffix} [queue]");
                    } else {
                        $localeSuffix = $locale !== null ? ", locale={$locale}" : ', locale=all';
                        $this->line("Would purge presentationItem: {$moduleName}::{$routeName} {$modelLabel} (id={$record->getKey()}){$localeSuffix} [sync]");
                    }
        ...
        if ($useQueue) {
            $this->line("Would queue PurgePresentationItemJob: {$moduleName}::{$routeName} ({$total} records) [{$modeLabel}, locale={$localeLabel}]");
        } else {
            $this->line("Would purge presentationItem: {$moduleName}::{$routeName} ({$total} records) [{$modeLabel}, locale={$localeLabel}]");
        }
    ...
    protected function resolveUseQueue(): bool
    {
        ...
        $this->warn("Queue unavailable (driver={$driver}), purging synchronously");
