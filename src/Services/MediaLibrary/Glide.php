...
class Glide implements ImageServiceInterface
{
    ...
    public function __construct(Config $config, Application $app, Request $request)
    {
        ...
        $baseUrl = join('/', [
            rtrim($baseUrlHost, '/'),
            ltrim($this->config->get('unusual.glide.base_path'), '/'),
        ]);
        ...
        $this->server = ServerFactory::create([
            ...
            'source' => $this->config->get('unusual.glide.source'),
            'cache' => $this->config->get('unusual.glide.cache'),
            'cache_path_prefix' => $this->config->get('unusual.glide.cache_path_prefix'),
            'base_url' => $baseUrl,
            'presets' => $this->config->get('unusual.glide.presets', []),
            'driver' => $this->config->get('unusual.glide.driver')
        ]);

        $this->urlBuilder = UrlBuilderFactory::create(
            $baseUrl,
            $this->config->get('unusual.glide.use_signed_urls') ? $this->config->get('unusual.glide.sign_key') : null
        );
    }
    ...
    {
        if ($this->config->get('unusual.glide.use_signed_urls')) {
            SignatureFactory::create($this->config->get('unusual.glide.sign_key'))->validateRequest($this->config->get('unusual.glide.base_path') . '/' . $path, $this->request->all());
        }
    ...
    {
        $defaultParams = config('unusual.glide.default_params');
    ...
    {
        $defaultParams = config('unusual.glide.lqip_default_params');
    ...
    {
        $defaultParams = config('unusual.glide.social_default_params');
    ...
    {
        $defaultParams = config('unusual.glide.cms_default_params');
    ...
    {
        $originalMediaForExtensions = $this->config->get('unusual.glide.original_media_for_extensions');
        $addParamsToSvgs = $this->config->get('unusual.glide.add_params_to_svgs', false);
